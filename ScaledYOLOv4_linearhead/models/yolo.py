...
from models.conhead import Conhead
...
class Model(nn.Module):
    ...
    def forward(self, x, augment=False, profile=False, name='wang'):
        if augment:
            ...
            return self.forward_once(x, profile, name)  # single-scale inference, train

    def forward_once(self, orig_x, profile=False, name='wang'):
        con_factor = self.head(orig_x)#Ra Ga Ba Rb Gb Bb
        a_factor = torch.sigmoid(con_factor[:,:3]) * 2
        b_factor = torch.atan(con_factor[:,3:]) * 2 / math.pi
        
        head_x = orig_x * a_factor[:,:,None,None] + b_factor[:,:,None,None]
